	...
	for data := range c.collectCh {
		err = (func() error {
			...
			if dv.Kind() != reflect.Ptr || dv.IsNil() {
				return fmt.Errorf("expected a pointer, got %s: %w", dv.Kind(), "invalid entity type")
			}

			dv = dv.Elem()

			if dv.Kind() != reflect.Slice {
				return fmt.Errorf("expected a pointer to a slice, got %s: %w", dv.Kind(), "invalid entity type")
			}

			elemType := dv.Type().Elem()

			if elemType.Kind() != reflect.Struct {
				return fmt.Errorf("expected a pointer to a slice of structs, got a slice of %s: %w", elemType.Kind(), "invalid entity type")
			}
